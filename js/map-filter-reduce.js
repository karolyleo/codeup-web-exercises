...
let avgExp = users.reduce((totalExp, user)=>{
    let { yearsOfExperience } = user;
    return ( user === users[ users.length - 1 ] ) ? ( ( totalExp + yearsOfExperience ) / users.length ) : ( totalExp += yearsOfExperience );
},0);
// console.log(avgExp)
